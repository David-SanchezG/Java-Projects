		...
		for(int i = 0;i <7;i++) {
			for (int j = 0; j<7; j++) {
				...
				if (solTab.tablero[i][j] == 1 ) {
					tablero[i][j].setIcon(new ImageIcon(getClass().getResource("images/solitairePion.gif")));
					}
				else tablero[i][j].setIcon(new ImageIcon(getClass().getResource("images/solitaireVide.gif")));
				}
			}
		}
	}
	...
	class Caja extends JButton implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cajaSelec == null && solTab.tablero[this.l][this.c] == 1)  {
				...
				boolean b = solTab.jugada(cajaSelec.l,cajaSelec.c,this.l,this.c);
				if (b) {
				cajaSelec.setBorder(null);
				cajaSelec = null;
				cambio();
				...
				cajaSelec.setBorder(null);
				cajaSelec = this;
				if (solTab.tablero[this.l][this.c] == 1)this.setBorder(BorderFactory.createLineBorder(Color.red,3));
				}
				}
			}}
		}
